function [X, Y, M_tilde, Dt, B, nTrials] = SingleTrial_SVD(varargin)

% Perform Singular Value Decomposition (SVD) prior to estimating Principal 
% Directions of Mediation (PDM)
%
% :Usage:
% ::
%       [x, y, M_tilde, Dt, B, nTrials] = SingleTrial_SVD(files, B, varargin); 
% 
%           ---OR---
% 
%       [x, y, M_tilde, Dt B, nTrials] = SingleTrial_SVD(x,y,M, varargin); 
%
% :Inputs:
%
%   **files**
...
%
%   **Mtilde**
%       mediator after SVD (N X B matrix)
%
%   **Dt**
...
fprintf('SVD for %d subjects with %d components\n',N,B);
...
MM = M'*M;
[U, S, ~] = svd(MM,0);
S_inv= diag(1./sqrt(diag(S)));
US_inv = U*S_inv;
S_sqrt= diag(sqrt(diag(S)));
