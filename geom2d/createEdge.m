...
%
%
%   e = CREATEEDGE(p1, p2) return the edge between the two given
%   points.
%   
%   e = CREATEEDGE(x0, y0, dx, dy) the edge going through point (x0, y0)
%   and with direction vector(dx, dy).
%
%   e = CREATEEDGE(param) where param is an array of 4 values, create the
%   line oing through the point (param(1) param(2)), and with direction
%   vector (param(3) param(4)).
%   
%   e = CREATEEDGE(line, d) create the edge contained in 'line', with
%   same direction and start point, but with length given by 'd'.
%
%
