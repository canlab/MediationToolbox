function [paths, varargout] = mediation_threepaths_singlelevel(X, Y, M1, M2, varargin)
...
% Taylor et al.(2007)'s three path mediation analysis. For now, this is a separate function
% for single-level threepath mediation. X, Y, M1, and M2 vectors with one datapoint in each row per subject.
% The default for bootsamples is 1000.
% 
...
%
% Example)
%   X = rand(50,1); M1 = rand(50,1); M2 = rand(50, 1); 
%   Y = rand(50,1); 
% end
% 
% [paths, stats] = mediation_threepaths(X, Y, M1, M2, ...
%       'verbose', 'bootsamples', 10000);
% 
% Wani Woo, 11/04/2012
% Stephan Geuter 12/10/2015
%
% Reference: 
...
% choose multilevel or singlelevel
if iscell(Y), 
    N = length(Y);
else
    N = size(Y, 2);
...
for i = 1:length(varargin)
    if ischar(varargin{i})
        switch varargin{i}
            ...
            case 'bootsamples', bootsamples = varargin{i+1}; boottop = 1; 
            case {'boottop','bootstrap'}, boottop = 1; 
...
if verbose
    ...
    fprintf('Predictor (X): %s, Outcome (Y): %s\nMediator 1 (M1): %s, Mediator 2 (M2): %s\n', vnames{1}, vnames{2}, vnames{3},vnames{4});
    ...
    if N > 1 && domultilev
        ...
        fprintf('Options:\n\tBootstrap: %s\n\tLogistic(Y): %s\n',...
            nms{boottop+1}, nms{logistic_Y+1});
    end
end
...
if N > 1 && domultilev
    ...
    % added for single-level 3path mediation. seems to be unused. SG
    mediationfun = @fast_ols_b1b2b3;
end
...
for i = 1:N
    ...
    if iscell(M1), m1 = M1{i}; else m1 = M1(:,i); end
    if iscell(M2), m2 = M2{i}; else m2 = M2(:,i); end
    ...
    [paths(i,:), b1betas{i}, b2betas{i}, b3betas{i}, cpbetas{i}, cbetas{i}, sterrs(i,:), intcpt, n(i), residm1{i}, residm2{i}, residy{i}, residy2{i},...
        Vxm1(i),Vm1m2(i),Vm2y(i),Vxy(i)] = mediation_path_coefficients_threepaths_singlelevel(x, y, m1, m2, domultilev, logistic_Y);
    
    % Multilevel only: save std. deviations for getting standardized paths
    if N > 1, nrm(i, 1) = std(x) ./ std(y); end
    
    % bootstrap for single-level models
    if boottop && N == 1
        if logistic_Y
            error('single-level bootstrapping with logistic Y not yet implemented');
        else
            ...
            if isempty(mediation_covariates)
                bootpaths = bootstrp_havesamples(bootsam, mediationfun, x, y, m1, m2, intcpt);
            else
                bootpaths = bootstrp_havesamples(bootsam, mediationfun, x, y, m1, m2, intcpt, mediation_covariates);
            end
            ...
            stats.vnames = vnames;
             ...
             if isempty(mediation_covariates)
                [stats.p, stats.z] = bootbca_pval(0, mediationfun, bootpaths, paths(i,:), x,y, m1, m2, intcpt);
                stats.CI = bootbca_ci(.025, mediationfun, bootpaths, paths(i,:), x, y, m1, m2, intcpt)';
             end

             stats.biascorrect = 'p-values bias corrected';
    ...
    else  % get OLS stats if no bootstrap
...
% -------------------------------------------------------------------------
    function add_boot_samples_needed()
        ...
        if Be > 0
            ...
            else    % single-level 3path mediation. SG
                
                if dorobust || ~isempty(mediation_covariates)
                    error('not implemented yet');
                else
                    bootpaths = [bootpaths; bootstrp_havesamples(bootsam_Be, @fast_ols_b1b2b3, x, y, m1, m2, intcpt)];
                end
...
% -------------------------------------------------------------------------
% Compute critical OLS 3-path mediation in a faster way, for bootstrap
% -------------------------------------------------------------------------
function paths = fast_ols_b1b2b3(x, y, m1, m2, intcpt, varargin)
...
px = inv(xx'*xx)*xx'; % X beta-forming matrix
m1x = [m1 xx];
pm1x = inv(m1x'*m1x)*m1x'; % M1+X beta-forming matrix
m1m2x = [m1 m2 xx];
pm1m2x = inv(m1m2x'*m1m2x)*m1m2x'; % M1+M2+X beta-forming matrix

% Eq. and b# follow Taylor et al 2008
% Is M1 related to the X ?
% Eq. 1,  b2 = M2 ~ X + M1
b1paths = px * m1;    % predictors x mediators
b1 = b1paths(1,:);   % slopes for x ->  M1
...
% Eq. 3, [b3, c'] = Y ~ X, M1, M2
b3paths = pm1m2x * y;
b3 = b3paths(2,:);  % Y ~ M2, controlled for X, M1
cpbetas = b3paths(3, :); % Y ~ X, controlled for M1, M2
...
% 3-path mediation coeff
b1b2b3 = b1 .* b2 .* b3;


paths = [b1 b2 b3 cpbetas c b1b2b3];
