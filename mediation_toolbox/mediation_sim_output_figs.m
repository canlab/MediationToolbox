function mediation_sim_output_figs(wh_col, file_to_load, L2M)
    % mediation_sim_output_figs(wh_col, [file_to_load], [L2M])
    %
    % goes with output from mediation_sim1
    % just go to directory and run

    if(~exist('file_to_load', 'var') || isempty(file_to_load))
        file_to_load = 'mediation_sim_output';
    end
    
    if(~exist('L2M', 'var') || isempty(L2M))
        L2M = false;
    end
    ...
    for i = 1:length(pthr)
        if ~L2M
            ...
            whnull = nullp_L2M(:, wh_col) < pthr(i);
            whalt = altp_L2M(:, wh_col) < pthr(i);
        end
    ...
    if ~L2M
        ...
        saveas(gcf,['rocplot_' names{wh_col} '_L2M'],'png');
    end
